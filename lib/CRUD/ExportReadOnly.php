<?php
class CRUD_ExportReadOnly extends CRUD_ReadOnly {
    ...
    function setModel($a, $b =null,$c=null){
        ...
        if ($this->grid){
            $this->grid->addButton("Export CSV")->js("click")->univ()->redirect($this->api->getDestinationURL(null,
                            array("export_csv" => $this->grid->name)));
            $this->grid->addButton("Export PDF")->js("click")->univ()->redirect($this->api->getDestinationURL(null,
                            array("export_pdf" => $this->grid->name)));

            $this->grid->addButton("Export XLS")->js("click")->univ()->redirect($this->api->getDestinationURL(null,
